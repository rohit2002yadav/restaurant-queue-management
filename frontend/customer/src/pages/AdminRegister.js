...
import { Mail, Lock, User, Phone, Utensils, Building } from 'lucide-react';
import API from '../api';
import Input from '../components/Input';
import Button from '../components/Button';

export default function AdminRegister() {
    const [form, setForm] = useState({ restaurant_name: '', name: '', phone: '', email: '', password: '', confirm_password: '' });
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <Input label="Restaurant Name" icon={Building} placeholder="e.g. Rohit's Bistro"
                            value={form.restaurant_name} onChange={e => setForm({ ...form, restaurant_name: e.target.value })}
                            error={errors.restaurant_name} required />
                        <Input label="Your Name" icon={User} placeholder="Full name"
