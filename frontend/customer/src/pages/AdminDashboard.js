...
export default function AdminDashboard() {
    ...
    const avgWait = queue.length
        ? Math.round(queue.reduce((s, q) => s + (q.estimated_wait_mins || 0), 0) / queue.length)
        : 0;
        ...
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={fetchQueue}
                            className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 transition-all" title="Refresh">
                        ...
                        <button onClick={handleLogout}
                            className="flex items-center gap-2 text-sm text-gray-500 hover:text-red-500 transition-all px-3 py-2 rounded-lg hover:bg-red-50">
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    ...
                    <StatCard icon={Clock} label="Avg Wait" value={`${avgWait}m`} color="bg-green-500" />
                </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-50 text-left">
                                            {['Token', 'Customer', 'Party', 'Queue Type', 'Wait Time', 'Status', 'Action'].map(h => (
                                        ...
                                        {queue.map(entry => (
                                            <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <span className="font-bold text-orange-500">{entry.token_number}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4">
                                                    <Badge variant="default">{entry.queue_type}</Badge>
                                                </td>
                            ...
                            <div className="md:hidden divide-y divide-gray-100">
                                {queue.map(entry => (
                                    <div key={entry.id} className="p-4">
                                        ...
                                        {entry.special_request && <p className="text-xs text-gray-400 mt-1">Note: {entry.special_request}</p>}
                                        <button
                                            onClick={() => handleCall(entry.id)}
                                            disabled={actionLoading === entry.id || entry.status !== 'waiting'}
